...
func testAriesWithCurve(id math.CurveID, translator idemix1.Translator) {
	...
	Describe("setting up the environment with one issuer and one user with curve", func() {
		var (
			...
			blindCredReqOpts *bccsp.IdemixBlindCredentialRequestSignerOpts
		)
		...
		BeforeEach(func() {
			...
			Expect(raw).To(HaveLen(2 * curve.CoordByteSize))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(n).To(BeEquivalentTo(curve.ScalarByteSize))
			Expect(err).NotTo(HaveOccurred())
		...
		Describe("importing bases", func() {
			...
			It("returns the bases correctly", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(keyImportOpts.CommitmentBases).To(HaveLen(3))
			})
		...
		Describe("producing an idemix signature with an eid nym and rh nym", func() {
			...
			It("the signature is not valid when we expect both an eid nym and rh nym and supply the right eid nym and the wrong rh nym", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("signature invalid: rh nym validation failed, signature rh nym does not match metadata"))
				Expect(valid).To(BeFalse())
			})
			...
			It("the signature is not valid when we expect both an eid nym and rh nym and supply the right eid nym and garbage rh nym", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("signature invalid: rh nym validation failed, failed to unmarshal meta rh nym"))
				Expect(valid).To(BeFalse())
			})
		...
		Describe("Idemix Bridge Load", func() {
			Describe("setting up the environment with one issuer and one user", func() {
				...
				BeforeEach(func() {
					...
					Expect(err).NotTo(HaveOccurred())
					Expect(rawNymKeyPk).To(HaveLen(2 * curve.CoordByteSize))
