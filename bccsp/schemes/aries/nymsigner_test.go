...
func TestNymSigner(t *testing.T) {
	...
	cb.Add(ipk.PKwG.H[aries.UserSecretKeyIndex], sk)
	nym := cb.Build()

	sig, err := signer.Sign(sk, nym, rNym, _ipk, []byte("ciao"))
	assert.NoError(t, err)

	err = signer.Verify(_ipk, nym, sig, []byte("ciao"))
	assert.NoError(t, err)
	...
	err = signer.Verify(_ipk, curve.GenG1.Mul(curve.NewRandomZr(rand)), sig, []byte("ciao"))
	assert.EqualError(t, err, "contribution is not zero")
}
