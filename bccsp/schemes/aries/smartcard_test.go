...
func TestAll(t *testing.T) {
	...
	// convert public key
	pkbbs, err := bbs12381g2pub.NewBBSLib(curve).UnmarshalPublicKey(pubKey.PointG2.Compressed())
	assert.NoError(t, err)
	...
	pok_, err := bbs12381g2pub.NewBBSLib(curve).NewPoKOfSignature(sig, messagesFr, []int{1, 2}, pkwgbbs)
	assert.NoError(t, err)
	...
	err = pok.Verify(c, pkwgbbs, map[int]*bbs12381g2pub.SignatureMessage{1: {}, 2: {}}, []*bbs12381g2pub.SignatureMessage{messagesFr[1], messagesFr[2]})
	assert.NoError(t, err)
	...
	pok_, err = bbs12381g2pub.NewBBSLib(curve).NewPoKOfSignatureExt(sig, messagesFr[1:], []int{0, 1}, pkwgbbs, B, r, C)
	assert.NoError(t, err)
	...
	err = pok.VerifyExt(c, pkwgbbs, map[int]*bbs12381g2pub.SignatureMessage{1: {}, 2: {}}, []*bbs12381g2pub.SignatureMessage{messagesFr[1], messagesFr[2]}, B)
	assert.NoError(t, err)
	...
	// convert POK
	payloadnew := bbs12381g2pub.NewPoKPayload(messagesCount, []int{1, 2})
	payloadBytesnew, err := payloadnew.ToBytes()
	assert.NoError(t, err)
	signatureProofBytesnew := append(payloadBytesnew, proofbbs.ToBytes()...)
	payloadnew, err = bbs12381g2pub.ParsePoKPayload(signatureProofBytesnew)
	assert.NoError(t, err)
	signatureProofnew, err := bbs12381g2pub.NewBBSLib(curve).ParseSignatureProof(signatureProofBytesnew[payloadnew.LenInBytes():])
	assert.NoError(t, err)

	// verify proof with old code
	err = signatureProofnew.VerifyExt(c, pkwgbbs, map[int]*bbs12381g2pub.SignatureMessage{1: {}, 2: {}}, []*bbs12381g2pub.SignatureMessage{messagesFr[1], messagesFr[2]}, B)
	assert.NoError(t, err)
	
